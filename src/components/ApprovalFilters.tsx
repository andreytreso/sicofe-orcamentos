...
export function ApprovalFilters({
  ...
  return <Card className="mb-6 bg-white border border-gray-200">
      <CardContent className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700">Empresa *</label>
            <Select value={filters.empresaId} onValueChange={value => handleFilterChange('empresaId', value)} disabled={isLoadingEmpresas}>
              ...
              <SelectContent className="bg-white border border-gray-200 rounded-md shadow-lg z-50">
                {empresas.map(empresa => <SelectItem key={empresa.id} value={empresa.id} className="hover:bg-gray-50 focus:bg-gray-50 cursor-pointer px-3 py-2 text-gray-900">
                    {empresa.nome}
                  </SelectItem>)}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            ...
            <Select value={filters.status} onValueChange={value => handleFilterChange('status', value as ApprovalFilter['status'])}>
              <SelectTrigger className="h-10 bg-white border border-gray-300 hover:border-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:ring-opacity-50 rounded-md shadow-sm">
          ...
          <div className="flex items-end">
            <Button onClick={onSearch} disabled={isSearchDisabled} className="w-full bg-sicofe-blue hover:bg-sicofe-blue-dark text-white">
              {isSearching ? <>
        ...
        {!filters.empresaId && <p className="text-sm text-red-600 mt-2">* Selecione uma empresa para habilitar a busca</p>}
      </CardContent>
    </Card>;
}